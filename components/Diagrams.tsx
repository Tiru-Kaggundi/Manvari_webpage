...
export const ServiceCard: React.FC<ServiceCardProps> = ({ title, description, icon, delay }) => {
  return (
    <motion.div 
      ...
      className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300 hover:border-accent-DEFAULT/30 group"
    >
      <div className="w-12 h-12 bg-slate-50 rounded-lg flex items-center justify-center mb-6 text-primary-light group-hover:text-accent-DEFAULT group-hover:bg-accent-DEFAULT/10 transition-colors">
        {icon}
      </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-4xl mx-auto">
      {stats.map((stat, index) => (
        <div key={index} className="flex flex-col items-center p-6 bg-white rounded-lg border border-slate-100 shadow-sm">
          <div className="text-accent-DEFAULT mb-2 opacity-80">{stat.icon}</div>
